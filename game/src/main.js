...
export default class Game {
        ...
        try {
            ...
            if (customThreeObject) {
                ...
                if (material == null) {
                    ...
                    threeObject = this.getThreeObjectForBody(
                        body,
                        color,
                        material,
                    );
    ...
    getThreeObjectForBody(
        ...
        switch (shape.GetSubType()) {
            case this.Jolt.EShapeSubType_Box:
                const boxShape = this.Jolt.castObject(
                    shape,
                    this.Jolt.BoxShape,
                );
                const extent = this.wrapVec3(
            ...
            case this.Jolt.EShapeSubType_Sphere:
                const sphereShape = this.Jolt.castObject(
                    shape,
                    this.Jolt.SphereShape,
                );
                threeObject = new THREE.Mesh(
    ...
    createTriangleListFromThreeObject(threeObject) {
        ...
        threeObject.traverse((child) => {
            if (child.isMesh && child.geometry) {
                ...
                if (!positionAttribute) {
                    console.warn(
                        `Mesh ${child.name} missing position attribute`,
                    );
                    return;
                }
        ...
        for (let i = 0; i < indices.length; i += 3) {
            ...
            if (
                ...
                const v1 = new this.Jolt.Vec3(
                    vertices[idx1 * 3],
                    vertices[idx1 * 3 + 1],
                    vertices[idx1 * 3 + 2],
                );
                const v2 = new this.Jolt.Vec3(
                    vertices[idx2 * 3],
                    vertices[idx2 * 3 + 1],
                    vertices[idx2 * 3 + 2],
                );
                const v3 = new this.Jolt.Vec3(
                    vertices[idx3 * 3],
                    vertices[idx3 * 3 + 1],
                    vertices[idx3 * 3 + 2],
                );
    ...
    async createMap() {
        ...
        try {
            ...
            const mapShapeSettings = new this.Jolt.MeshShapeSettings(
                triangleList,
            );
    ...
    async createRoads() {
        ...
        try {
            ...
            const roadsShapeSettings = new this.Jolt.MeshShapeSettings(
                triangleList,
            );
    ...
    async createBuildings() {
        const gltf = await this.gltfLoader.loadAsync(
            "static/gameAssets/buildings.glb",
        );
    ...
    createVehicle() {
        ...
        try {
            ...
            Promise.all([
                ...
                const vehicleMesh =
                    this.dynamicObjects[this.dynamicObjects.length - 1];
                ...
                const mWheels = [];
                {
                    ...
                    fl.mPosition = new this.Jolt.Vec3(
                        halfVehicleWidth + wheelOffset,
                        -wheelOffsetVertical,
                        wheelBase + wheelOffsetLongitudal,
                    );
                    fl.set_mMaxSteerAngle(maxSteerAngle);
                    fl.mMaxHandBrakeTorque = 0.0;
                    fl.set_mLateralFriction(
                        this.createFrictionCurve(frontTyreLateralFriction),
                    );
                    fl.set_mLongitudinalFriction(
                        this.createFrictionCurve(frontTyreLongitudalFriction),
                    );
                    ...
                    fr.mPosition = new this.Jolt.Vec3(
                        -(halfVehicleWidth + wheelOffset),
                        -wheelOffsetVertical,
                        wheelBase + wheelOffsetLongitudal,
                    );
                    ...
                    fr.set_mLateralFriction(
                        this.createFrictionCurve(frontTyreLateralFriction),
                    );
                    fr.set_mLongitudinalFriction(
                        this.createFrictionCurve(frontTyreLongitudalFriction),
                    );
                    ...
                    bl.mPosition = new this.Jolt.Vec3(
                        halfVehicleWidth + wheelOffset,
                        -wheelOffsetVertical,
                        -wheelBase + wheelOffsetLongitudal / 2,
                    );
                    bl.set_mMaxSteerAngle(0);
                    fr.mMaxHandBrakeTorque = 100.0;
                    bl.set_mLateralFriction(
                        this.createFrictionCurve(rearTyreLateralFriction),
                    );
                    bl.set_mLongitudinalFriction(
                        this.createFrictionCurve(rearTyreLongitudalFriction),
                    );
                    vehicle.mWheels.push_back(bl);
                    mWheels.push(bl);

                    const br = new this.Jolt.WheelSettingsWV();
                    br.mPosition = new this.Jolt.Vec3(
                        -(halfVehicleWidth + wheelOffset),
                        -wheelOffsetVertical,
                        -wheelBase + wheelOffsetLongitudal / 2,
                    );
                    ...
                    fr.mMaxHandBrakeTorque = 100.0;
                    br.set_mLateralFriction(
                        this.createFrictionCurve(rearTyreLateralFriction),
                    );
                    br.set_mLongitudinalFriction(
                        this.createFrictionCurve(rearTyreLongitudalFriction),
                    );
                ...
                mWheels.forEach((wheelS) => {
                    ...
                    wheelS.set_mSuspensionMinLength(
                        wheelS.get_mSuspensionMinLength() * suspensionMinLength,
                    );
                    wheelS.set_mSuspensionMaxLength(
                        wheelS.get_mSuspensionMaxLength() * suspensionMaxLength,
                    );
                    wheelS.set_mSuspensionPreloadLength(
                        wheelS.get_mSuspensionPreloadLength() *
                        suspensionPreloadLenght,
                    );

                    const spring = wheelS.get_mSuspensionSpring();
                    spring.set_mStiffness(
                        spring.get_mStiffness() * suspensionStiffness,
                    );
                    spring.set_mFrequency(
                        spring.get_mFrequency() * suspensionFrequency,
                    );
                    spring.set_mDamping(
                        spring.get_mDamping() * suspensionDamping,
                    );
                });
                ...
                if (fourWheelDrive) {
                    ...
                    controllerSettings.mDifferentials.push_back(
                        frontWheelDrive,
                    );
                }
                ...
                this.vehicle = new this.Jolt.VehicleConstraint(
                    carBody,
                    vehicle,
                );
                const tester = new this.Jolt.VehicleCollisionTesterCastCylinder(
                    LAYER_MOVING,
                    0.05,
                );
                ...
                callbacks.GetCombinedFriction = (
                    ...
                    const otherBody = this.Jolt.wrapPointer(
                        body2,
                        this.Jolt.Body,
                    );
                ...
                this.playerController = this.Jolt.castObject(
                    this.vehicle.GetController(),
                ...
                controllerCallbacks.OnTireMaxImpulseCallback = (
                    ...
                    const resultObj = this.Jolt.wrapPointer(
                        result,
                ...
                const createThreeWheel = (constraint, wheelIndex, body) => {
                    ...
                    const controlArm = this.createUnitCylinder(
                        0.04,
                        armMaterial,
                        8,
                    );
                    ...
                    const updater = {
                        ...
                        updateLocalTransform: () => {
                            try {
                                ...
                                const wheelPos = this.wrapVec3(
                                    transform.GetTranslation(),
                                );
                                wheel.position.copy(wheelPos);
                                const wheelQuat = this.wrapQuat(
                                    transform.GetRotation().GetQuaternion(),
                                );
                                wheel.quaternion.copy(wheelQuat);
                                if (isLeftSide) wheel.rotateX(-Math.PI / 2);
                                else wheel.rotateX(Math.PI / 2);
                                ...
                                if (length < 1e-6) controlArm.visible = false;
                                    ...
                                    controlArm.scale.set(
                                        1,
                                        length / controlArm.userData.unitLength,
                                        1,
                                    );
                                }
                            } catch (err) {
                ...
                for (let i = 0; i < vehicle.mWheels.size(); i++) {
                    const updater = createThreeWheel(
                        this.vehicle,
                        i,
                        vehicleMesh,
                    );
    ...
    prePhysicsUpdate() {
        ...
        try {
            ...
            const vehicleRotation = this.wrapQuat(
                this.vehicleBody.GetRotation(),
            );
            this.playerRotation = {
            ...
            if (this.previousForward * forward < 0.0) {
                const rotation = this.wrapQuat(
                    this.vehicleBody.GetRotation().Conjugated(),
                );
            ...
            this.playerController.SetDriverInput(
                forward,
                right,
                brake,
                handBrake,
            ...
            if (this.vehicleWheels && this.vehicleWheels.length) {
                this.vehicleWheels.forEach((entry) => {
                    if (
                        entry &&
                        typeof entry.updateLocalTransform === "function"
                    ) {
            ...
            for (let i = 0, il = this.dynamicObjects.length; i < il; i++) {
                ...
                if (body.GetBodyType() == this.Jolt.EBodyType_SoftBody) {
                    if (objThree.userData.updateVertex) {
                        ...
                        objThree.geometry = this.createMeshForShape(
                            body.GetShape(),
                        );
                    }
                }
